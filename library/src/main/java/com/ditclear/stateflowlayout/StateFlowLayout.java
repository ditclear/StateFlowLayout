        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (i == 0 || i == getChildCount() - 1) {
                ...
                if (child instanceof Node) {
                    ...
                    plusWidth += (width > rect.width() ? width : rect.width()) / 2;
                    plusHeight += height + rect.height();
                }
            }
        }
...
//        }
        ...
        if (widthMode == MeasureSpec.AT_MOST && heightMode == MeasureSpec.AT_MOST) {
            setMeasuredDimension(getMeasuredWidth() + plusWidth,
                    getMeasuredHeight() + plusHeight);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (child instanceof Node) {
                ...
                String state = node.getState();
                String content = node.getContent();
                Log.d(TAG, "onDraw: " + state + "  " + content);
                if (state != null) {
                    mStatePaint.getTextBounds(state, 0, state.length(), rect);
